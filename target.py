...
class TargetCrop:
    ...
    checkpoint_file = (
        "checkpoints/scnet/scnet_r50_fpn_1x_coco-c3f09857.pth"  # 预训练模型加载路径
    )
    device = "cpu"  # cpu|gpu
    ...
    def gen_target_crop(imgUrl):
        res = {"b64": "", "score": ""}
        ################ 图片 处理 ################
        img_raw, result = gen_result(imgUrl)
